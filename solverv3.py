...
def is_valid_col_constraint(puzzle, stateInfo):
    ...
    for col in range(numCols):
        ...
        if re.fullmatch(pattern, stringToMatch) is None:
            return False
...
def generate_possible_successors(puzzle, state):
    ...
    for row in range(puzzle.row):
        ...
        if tracked_records[row][0] == len(puzzle.row_list[row]) - 1:
            continue
        if tracked_records[row][0] < len(puzzle.row_list[row]) - 1:
            constraint_ind = tracked_records[row][0] + 1
            startCol = tracked_records[row][1] + 2
            if constraint_ind >= len(puzzle.row_list[row]) or startCol >= puzzle.col: # all constraints are satisfied for this row
                continue
            ...
            for col in range(startCol, puzzle.col):
                ...
                indToInsert = rowStart + col
                newState = (state.state + '.')[:-1] #state.state
                newTracker = state.track_row_constraints[:]
                colInRow = col
                for constraint in puzzle.row_list[row][constraint_ind:]:
                    newState = turn_on(newState, indToInsert, constraint)
                    colInRow += constraint - 1
                    newTracker[row] = [constraint_ind, colInRow]

                    # we only want to iterate more to completely fill this row if colsLeft == requiredSpaces
                    # so break if there are more possibilities
                    if colsLeft > requiredSpaces:
                        break

                    constraint_ind += 1
                    colInRow += 2
                    indToInsert += constraint + 1

                successor = StateInfo(newState, newTracker)
                successors.append(successor)
                if colsLeft == requiredSpaces: # if we already filled in all constraints in the current row
                    break
